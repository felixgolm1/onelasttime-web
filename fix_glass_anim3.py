...
# Instead of modifying the raw HTML, we can just select them via CSS in GSAP!
# glass-who items: '#glass-who p, #glass-who li, #glass-who hr'
# glass-objectives items: '#glass-objectives p, #glass-objectives li, #glass-objectives hr'

# We just need to split the titles.
# In the JS, near splitFltChars:
split_logic = '''      function splitFltChars(selector) {
        const chars = [];
...
content = re.sub(r'      function splitFltChars.*?const rightChars = splitFltChars\(\'#flip-right-text \.flt-in-up\'\);', split_logic, content, flags=re.DOTALL)
