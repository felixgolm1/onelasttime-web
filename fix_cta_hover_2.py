...
target_regex = r"\s*document\.addEventListener\('mouseover', \(e\) => \{\s*const cta = e\.target\.closest.*?window\._ctaHoverActive = true;\s*\}\);\s*document\.addEventListener\('mouseout', \(e\) => \{\s*const cta = e\.target\.closest.*?window\._ctaHoverActive = false;\s*\}\s*\}\);"
...
if 'Efecto de lucecitas exclusivas dentro del CTA' not in content:
