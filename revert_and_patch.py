...
text = re.sub(r"// --- PORTAL EFFECT & BRAIN ---.*?\} else \{[^\}]*\}\s*(?=\}\s*\}\s*else)", "", text, flags=re.DOTALL)
text = re.sub(r"if\(typeof brainUniforms !== 'undefined'\) brainUniforms\.uTime\.value = t;\s*\n\s*", "", text)

# Now, we should be clean. Let's verify by checking if PORTAL EFFECT is still there.
if "PORTAL EFFECT" in text:
    print("WARNING: Portal effect still found, manual clean needed")
    # aggressive clean:
...
load_glb = """
    gltfLoader.load('assets/models/human_brain.glb',
      (gltf) => {
        ...
        // Ajuste de tamaño para que el cerebro se vea majestuoso y centrado
        const targetW = 20.0; 
        ...
        // Z = -15 para estar en buen plano visible de la camara (la camara est en 0,0,0 aprox o 0,0,10)
...
# 5. Scroll Logic
# We insert right after lineR.setAttribute
target = "lineR.setAttribute('y1', '100%'); lineR.setAttribute('y2', `${100 - (100 * dashP)}%`);\n        }"
scroll_logic = """
        // --- PORTAL EFFECT & BRAIN ---
        // Se activa slo a partir de 11.62, dejando la revista visible y esttica de 10.62 a 11.62
        let portalP = mapRange(prog, 11.62, 12.62, 0, 1);
        if (portalP > 0) {
            let magScale = 1 + (portalP * 12);
            let magOpacity = mapRange(portalP, 0.4, 1.0, 1, 0);
            
            // Fix critico: Slo modificamos transform y dejamos que herede su position original, pero preservando translate(-50%,-50%) si lo necesitaba (aunque el container padre no lo tiene).
            // Lo ms seguro es NO tocar el magazineScene entero, sino modificar sus tracks internos para hacer el dive-in, o usar scale3d en el scene.
            // PERO vamos a usar una variable custom o css var para no machacar estilos inline anteriores.
            // Para asegurar, lo mejor es escalar el opacity del magazineScene y punto.
            ...
            // Restore scale limpiando la propiedad inline para que herede del CSS (el scale(0.85) u otros)
