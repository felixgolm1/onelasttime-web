...
# 1. Copy the file
shutil.copy('terminos.html', 'garantia.html')
...
# Change Title
text = text.replace('<title>Términos y Condiciones - One Last Time</title>', '<title>Garantía de Devolución - One Last Time</title>')
text = text.replace('<h1>Términos y Condiciones / Aviso Legal</h1>', '<h1>Garantía de Devolución</h1>')
...
text = re.sub(r'(<div class="container">\s*)<h2.*?(?=    <div style="display: flex; justify-content: center; margin-top: 80px;)', r'\g<1>' + body_html, text, flags=re.DOTALL)

with open('garantia.html', 'w', encoding='utf-8') as f:
    f.write(text)
    
print('Created garantia.html')
